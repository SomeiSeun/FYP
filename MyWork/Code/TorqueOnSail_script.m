...
angle_spinaxis = 45; % Orientation about spin axis (clockwise)
...
radius = AU*0.1194*1000; % in meters
...
%{
side = sqrt(28.6);
angle = 30; % Orientation about spin axis (clockwise)
theta = 360-angle; % Anti-clockwise value
dist = abs(side*cosd(theta) - side*sind(theta));

local_coords = linspace(-dist/2,dist/2);
...
x_centroid = dist*(2*SRP_local(end)+SRP_local(1))/(3*(SRP_local(1)+SRP_local(end))) - dist/2;
...
x = ones(100).*linspace(-side/2,side/2); % Axis from Sun to CoM of Sail
y = ones(100).*(linspace(side/2,-side/2)'); % Axis perpendicular to x through length of sail
...
title('SRP over Sail - body inertial frame')
xlabel('Radius axis [m]')
ylabel('Perpendicular axis [m]')
zlabel('SRP [\muPa]')
grid on
...
if (angle_spinaxis > 45 && angle_spinaxis < 135) || (angle_spinaxis > 225 && angle_spinaxis < 315)
    for i = 1:(length(x_new)-1)
        y_centroids(i,:) = (y_new(i,1:(end-1)) + y_new(i,2:end))/2;
        x_centroids(:,i) = (x_new(1:(end-1),i) + x_new(2:end,i))/2;
    end
else
    for i = 1:(length(x_new)-1)
        x_centroids(i,:) = (x_new(i,1:(end-1)) + x_new(i,2:end))/2;
        y_centroids(:,i) = (y_new(1:(end-1),i) + y_new(2:end,i))/2;
    end
end
...
force_tot = sum(force_local,'all').*cosd(Sun_Sail_angle); 
...
T1 = force_tot*X_CEN_force;
T2 = force_tot*Y_CEN_force;
...
omega_1_20y = omega_dot_1*(20*yr*day*60*60)
omega_2_20y = omega_dot_2*(20*yr*day*60*60)

angle_1_20y = (omega_dot_1*(20*yr*day*60*60)^2)/2
angle_2_20y = (omega_dot_2*(20*yr*day*60*60)^2)/2
